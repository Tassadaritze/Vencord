...
function PronounsChatComponent({ message, className }: { message: Message; className?: string; }) {
    ...
    return pronouns && (
        <Tooltip text={i18n.Messages.USER_PROFILE_PRONOUNS}>
            {tooltipProps => (
                <span
                    {...tooltipProps}
                    className={classes(styles.timestampInline, styles.timestamp, className)}
                >• {pronouns}</span>
            )}
        </Tooltip>
    );
...
export const CompactPronounsChatComponentWrapper = ErrorBoundary.wrap(({ message }: { message: Message; }) => {
    ...
    return <PronounsChatComponent message={message} className="vc-user-message-pronouns-compact" />;
}, { noop: true });
